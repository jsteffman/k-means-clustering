...
### `r fa("github", fill = "steelblue")` Github repository for this code: https://github.com/jsteffman/k-means-clustering

This will also eventually (...) be a more fully fleshed out tutorial on https://jsteffman.github.io/teaching.html. 
...
- Here, 30 time-normalised samples, speaker means for each of the 8 stimulus tunes 
...
<div style="float: left; width: 25%;">
#### Points are allocated to clusters based on proximity to seeds (Euclidian distance)
</div>
...
- In addition to the "best" k value, others may be of interest as informed by predictions or theory
  </div>
...
#### The key difference is in the understanding of DISTANCE between observations, where differences are considered at each time point when comparing two trajectories (L2 norm, or RMSD)
...
tcog_data %>% 
  ...
  ylab("normalised ERB")+xlab("prop. word duration")+
  geom_line(color="gray50",size=1)+facet_wrap(~id,nrow = 2)+ theme(strip.text = element_blank())+
...
scale_color_viridis(option = "D",end = 0.9,begin=0.3)+
  ...
  geom_line(color="gray50",size=0.8)+facet_wrap(~paste(rmsd,id))+ theme(strip.text = element_blank(),legend.position = "none", axis.text.x=element_blank(),
...
- Caliński Harabasz criterion, in agreement with others 


```{r, echo = F,message=F, fig.width=7.5, fig.height=4.5,fig.show='animate',animation.hook="gifski",interval = 1.5}
...
hc %>% select(-start,-end,-filename) -> hc
  
colnames(hc)<-c("id_tune",1:30,"cluster")

hc %>% gather("normtime","scaled_erb",2:31) %>% 
  mutate(normtime= as.numeric(normtime)) -> hc
